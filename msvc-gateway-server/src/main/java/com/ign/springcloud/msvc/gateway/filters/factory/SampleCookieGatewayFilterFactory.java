package com.ign.springcloud.msvc.gateway.filters.factory;
...
public class SampleCookieGatewayFilterFactory
	...
	public GatewayFilter apply(ConfigurationCookie config) {
		return new OrderedGatewayFilter((exchange, chain) -> {
			logger.info("Pre Executing cookie filter" + config.getMessage());
			return chain.filter(exchange).then(Mono.fromRunnable(() -> {
				Optional.ofNullable(config.value).ifPresent(val -> {
					exchange.getResponse().addCookie(ResponseCookie.from(config.name, val).build());
				});
				logger.info("Post Executing cookie filter" + config.getMessage());
			}));
		}, 100);
	}
